...
export default function ZoneQuizGameSidebar({ onSpeakGuide }: ZoneQuizGameSidebarProps) {
  return (
    <div className="zone-game-sidebar flex w-full shrink-0 flex-row items-center justify-between gap-2 overflow-hidden rounded-[20px] border border-[#c4c9d4] bg-white p-2.5 shadow-[0_8px_24px_rgba(0,76,110,0.12)] sm:gap-4 sm:p-4 xl:h-full xl:w-full xl:flex-col xl:rounded-[24px] xl:px-5 xl:pb-3 xl:pt-5">
      <div className="flex w-auto flex-col items-center gap-3 xl:w-full xl:gap-5">
        <button
          onClick={onSpeakGuide}
          className="flex min-h-11 shrink-0 items-center gap-2 rounded-[40px] bg-[#fea01f] px-3 py-2 font-vietnam text-[12px] font-medium text-white shadow-sm transition-all hover:bg-[#e08b15] active:scale-95 sm:px-4 sm:text-[14px] xl:text-[16px]"
        >
          <img src={zoneQuizAssets.speaker} alt="Speak" className="w-[24px] h-[24px] invert brightness-0" />
          <span>HƯỚNG DẪN CHƠI</span>
        </button>
        <div className="hidden w-full shrink-0 flex-col items-center rounded-[12px] bg-[#e5f2ff] p-4 xl:flex">
      ...
      <div className="flex min-w-0 flex-1 flex-col items-center gap-2 xl:w-full">
        <div className="zone-sidebar-bubble drop-shadow-[0px_4px_5px_rgba(0,0,0,0.1)] flex flex-col items-end w-full">
          ...
          <div className="relative hidden h-[25px] w-full shrink-0 xl:block">
